...
module.exports = function main (options, cb) {
  ...
  app.use(function fiveHundredHandler (err, req, res) {
    if (err.status >= 500) {
      logger.error(err)
    }
  ...
  server = app.listen(opts.port, opts.host, async function (err) {
    ...
    await mongoose.connect(config.get('mongoUri') ?? process.env.MONGO_URL, {
      useNewUrlParser: true,
      useUnifiedTopology: true,
      useCreateIndex: true
    });
    console.log(`App has been started on port ${opts.port}`);
